...
// ⚡ PERFORMANCE: Pre-create icons once - avoid creating new icons on every render
...
// ⚡ PERFORMANCE: Create icons once at module level
...
// ⚡ PERFORMANCE: Custom cluster icon with count
const createClusterCustomIcon = (cluster: any) => {
    const count = cluster.getChildCount();
    const isLarge = count > 50;
    const dimension = isLarge ? 50 : 30;
    const className = isLarge ? 'cluster-marker cluster-large' : 'cluster-marker cluster-small';
...
// ⚡ PERFORMANCE: Memoized single marker component
const CameraMarker = memo(({ 
    ...
    // Custom comparison - only re-render if selection changes or camera ID changes
    return prevProps.isSelected === nextProps.isSelected && 
           prevProps.camera._id === nextProps.camera._id;
});
...
export default function CameraMarkers({ 
    ...
    routingMode, 
    routingState, 
    onRoutingCameraClick, 
    heatEnabled, 
    routeCoordinates 
}: CameraMarkersProps) {
    const [cameras, setCameras] = useState<Camera[]>([]);
    const [loading, setLoading] = useState(true);
    const [expandedCameraIds, setExpandedCameraIds] = useState<Set<string>>(new Set());
    const map = useMap();
    ...
    // ⚡ PERFORMANCE: Throttled real-time updates (1 second instead of 500ms)
    useEffect(() => {
        let updateTimeout: NodeJS.Timeout | null = null;
        
        const unsubscribe = trafficApi.subscribe((trafficData) => {
            if (!trafficData.cameraId) return;
            ...
            // ⚡ PERFORMANCE: Batch updates every 1 second
            if (!updateTimeout) {
    ...
    // ⚡ PERFORMANCE: Memoize filtered cameras
    const displayCameras = useMemo(() => {
        if (!routeCoordinates || routeCoordinates.length === 0) {
            return cameras;
        }
        return cameras.filter(camera => isCameraNearRoute(camera, routeCoordinates));
    }, [cameras, routeCoordinates, isCameraNearRoute]);

    // ⚡ PERFORMANCE: Memoize click handler factory
    ...
    }, [routingMode, routingState, onRoutingCameraClick, onCameraClick]);

    // Handle cluster click - expand markers at their original positions
    const handleClusterClick = useCallback((cluster: any) => {
        ...
        childMarkers.forEach((marker: any) => {
            ...
            const camera = camerasRef.current.find(c => 
                c.loc.coordinates[1] === lat && c.loc.coordinates[0] === lng
            );
            if (camera) {
                cameraIds.add(camera._id);
            }
        });
        
        setExpandedCameraIds(cameraIds);
    }, []);

    // Close expanded markers when clicking on map
    useEffect(() => {
        const handleMapClick = () => {
            if (expandedCameraIds.size > 0) {
                setExpandedCameraIds(new Set());
            }
        };
    ...
    }, [map, expandedCameraIds]);

    // ⚡ PERFORMANCE: Only cluster when more than 100 cameras
    const shouldCluster = displayCameras.length > 100;

    if (loading || heatEnabled) {
        return null;
    }

    // If less than 100 cameras, render markers directly without clustering
    if (!shouldCluster) {
        return (
            <>
                {displayCameras.map((camera) => (
                    <CameraMarker
                        key={camera._id}
                        camera={camera}
                        isSelected={camera._id === selectedCameraId}
    ...
    // Split cameras into clustered and expanded
    const clusteredCameras = displayCameras.filter(c => !expandedCameraIds.has(c._id));
    const expandedCameras = displayCameras.filter(c => expandedCameraIds.has(c._id));

    return (
        <>
            {/* Expanded markers rendered outside cluster at their original positions */}
            {expandedCameras.map((camera) => (
                <CameraMarker
                    key={`expanded-${camera._id}`}
                    camera={camera}
                    isSelected={camera._id === selectedCameraId}
                    onClick={() => handleCameraClick(camera)}
                />
